...
class AVLTree(object):
    ...
    def height(self,node):
        if node is None:
            ...
            return node.height
    def singleLeftRotate(self,node):#向右单转
        k1 = node.left
        node.left = k1.right
        k1.right = node
        ...
        return k1
    def singleRightRotate(self,node):#向左单转
        k1 = node.right
        node.right = k1.left
        k1.left = node
        node.height = max(self.height(node.right),self.height(node.left)) + 1
...
number_list = (5,6,8,3,2,4,7)
tree = AVLTree()
for number in number_list:
    tree.put(number)
print("该树的前序遍历是：",end = "")
tree.preorder()
print("该树的最大元素是：",tree.findMax().key)
print("该树的最小元素是：",tree.findMin().key)
tree.delete(8)
print("删除元素8后前序遍历为：",end = "")
tree.preorder()
